...
const Camera = () => {
  ...
  axios.get("https://3.110.118.195/get_camera").then(res=>{
    setCameras(res.data)
  })
  }, [])
  ...
  const addCamera = () => {
    if (newCamera.camera_id && newCamera.camera_name) {
      ...
      axios.post("https://3.110.118.195/add_camera",newCamera).then(res=>{
    ...
    if (editingCamera) {
      setCameras(cameras.map((camera) => {
        if (camera.camera_id === editingCamera.camera_id) {
            ...
            axios.post('https://3.110.118.195/update_camera',editingCamera).then(res=>alert(res.data.message))
  ...
  const deleteCamera = (id) => {
    ...
    axios.post("https://3.110.118.195/delete_camera",{'camera_id':id}).then(res=>{
            alert(res.data.message)
        })
